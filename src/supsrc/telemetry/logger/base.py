...
def setup_logging(
    ...
    if not _is_tui_active and not file_only:
        ...
        slog.debug(
            "TUI active: Standard StreamHandler to sys.stdout is suppressed even if file_only is false."
        )
        ...
        print(
            f"WARNING: Logging configured with file_only=True but no log_file was provided. No log output will be generated.",
            file=sys.stderr,
        )
        ...
        slog.info(
            f"Console logging via StreamHandler suppressed (file_only=True). Logs are being written to '{log_file}'."
        )
    ...
    if log_file:
        try:
            ...
            logging.getLogger(BASE_LOGGER_NAME).error(
                f"Failed to setup file logging to '{log_file}': {e}", exc_info=True
            )
