...
def setup_logging(level: int = logging.INFO, json_logs: bool = False, log_file: Optional[str] = None) -> None:
    ...
    shared_processors: list[structlog.types.Processor] = [
        ...
        structlog.stdlib.remove_logger_module_info, # Optional: remove module/func info
    ...
    if json_logs:
        # Use JSON renderer for console
        final_console_renderer = structlog.processors.JSONRenderer()
    else:
        # Use colored console renderer for development
        final_console_renderer = structlog.dev.ConsoleRenderer(
            # Customize ConsoleRenderer output format slightly if needed
            # Example: "{timestamp} {level} {emoji} | {padded_logger} | {event} {event_kw}"
    ...
    formatter = structlog.stdlib.ProcessorFormatter(
        ...
        processor=final_console_renderer,
    )
    ...
    # Log initial message using a structlog logger
