    ...
    async def _consume_events(self) -> None:
        ...
        while not self.shutdown_event.is_set():
            ...
            try:
                ...
                # --- Check if repository is individually paused ---
                if repo_state and repo_state.is_paused:
                    consumer_log.info(
                        "⏸️ Repository is individually PAUSED, skipping event processing",
                        repo_id=repo_id,
                        event_type=event.event_type,
                    )
                    # Put the event back in the queue for later processing
                ...
                try:
                    ...
                    event_log.debug(f"Debug config: self.config={self.config}, self.config.repositories={self.config.repositories}")
                    repo_config = self.config.repositories.get(repo_id) if self.config else None
                    event_log.debug(f"Debug repo_config: repo_config={repo_config}")
