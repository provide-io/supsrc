# env.ps1 - supsrc Development Environment Setup
#
...
if (-not (Test-Path "pyproject.toml")) {
    ...
    Write-Host "Please run this script from the supsrc root directory"
    exit 1
}
...
$Profile = if ($env:SUPSRC_PROFILE) { $env:SUPSRC_PROFILE } else { "default" }
if ($Profile -eq "default") {
    $VenvDir = "workenv/supsrc_${TFOS}_${TFARCH}"
} else {
...
$LogDir = Join-Path $env:TEMP "supsrc_setup"
...
Write-Host "Installing supsrc in editable mode..." -NoNewline
try {
...
Write-Host "`n$("supsrc development environment activated" | Write-Host -ForegroundColor Green)"
Write-Host "Virtual environment: $VenvDir"
Write-Host "Profile: $Profile"
Write-Host "`nUseful commands:"
Write-Host "  supsrc --help  # supsrc CLI"
