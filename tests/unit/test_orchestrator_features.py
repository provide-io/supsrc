...
async def test_event_consumption_for_paused_repository():
    ...
    repo_id = "paused_repo"
    repo_state = RepositoryState(repo_id=repo_id, is_paused=True) # Set is_paused directly in constructor
    orchestrator.repo_states[repo_id] = repo_state

    # Create minimal actual config objects for the test
    from supsrc.config import SupsrcConfig, RepositoryConfig, GlobalConfig
    from pathlib import Path

    mock_repo_config = RepositoryConfig(
        path=Path("/tmp/test_repo"), # A dummy path
        repository={"type": "mock_engine"}, # A dummy engine type
        enabled=True,
        rule=MagicMock() # Rule can still be mocked
    )
    orchestrator.config = SupsrcConfig(
        global_config=GlobalConfig(),
        repositories={repo_id: mock_repo_config}
    )
